...
export class TownhouseContextService {
  ...
  private readonly _currentTownhouse = signal<TownhouseContextModel | null>(
    null,
  );
  ...
  loadBySlug(slug: string): Observable<TownhouseContextModel | null> {
    ...
    return this.http
      .get<TownhouseResponse>(
        `${API_BASE_URL}/townhouse/by-slug/${encodeURIComponent(normalizedSlug)}`,
      )
      .pipe(
        map((townhouse) => ({
          ...townhouse,
        ...
        tap((townhouse) =>
          this._finishLoading(normalizedSlug, townhouse, null),
        ),
  ...
  private _finishLoading(
    ...
    this.title.setTitle(
      townhouse ? `${townhouse.name} | Tangerine` : 'Tangerine',
    );
  }
}
